...
from feamodule.eval import get
...
def output_feature(seq_file,hex_file,species):
	...
	feature = open(seq_file+'.Genomic','w')
	out_label = 1
	...
	for seq in Seq.parse(seq_file,'fasta'):
		...
		if species == "Integrated":
			...
			feature.write("\t".join(map(str,[Cov,inte_fe,GC,insta_fe,Len,T0,fickett_fe,G2,C3,PI_fe,A3,C1,G3,hexamer,TG,G1,TC,A0,A1,AC,C2,G0,T4,C0,A4,G,A2,T,T3,G4,C4,gra_fe,T2,AG,AT,T1,A,C]))+"\n")
		# print >> tmp, out_label,
...
def evaluate(evaluate_file,name,mod):
	with open(evaluate_file,'a') as e:
		name,mod = os.path.basename(name),os.path.basename(mod)
		aupr,auc,f1,acc,recall,speci,precision = get('tmp.txt')
		e.write('\t'.join(map(str,[mod,name,aupr,auc,f1,acc,recall,speci,precision]))+'\n')
...
def main():
    ...
    prefix = os.path.splitext(args.RNA_file)[0]
    output_feature(args.RNA_file,args.hexmar,args.species)
    # predict(args.range,args.model)
    # merge(args.outfile)
    # evaluate('eval_cross',args.RNA_file,args.model)
    # deleted()
